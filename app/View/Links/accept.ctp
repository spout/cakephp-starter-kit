...
?>
<?php echo $this->Form->create($modelClass, array('url' => array('action' => 'accept', $item[$modelClass]['id'])));?>
<fieldset>
<legend><?php echo h($title);?></legend>
<?php 
$urlView = $this->Html->url(array('controller' => 'links', 'action' => 'view', 'id' => $item[$modelClass]['id'], 'slug' => slug($title)));
//$fullBaseUrl = FULL_BASE_URL;
$urlLinkToUs = $this->Html->url(array('controller' => 'pages', 'action' => 'display', 'faire-un-lien'));
$message = <<<EOT
Bonjour {$item['User']['firstname']} {$item['User']['lastname']},
...
EOT;
?>
<?php echo $this->Form->input('email', array('label' => __('A'), 'size' => 30, 'default' => $item['User']['email']));?>
