<?php if(isset($item[$modelClass])):?>
	...
	if ((isset($item[$modelClass]['user_id']) && Auth::id() == $item[$modelClass]['user_id']) || Auth::hasRole(ROLE_ADMIN)) {
		if (Auth::hasRole(ROLE_ADMIN) && isset($item[$modelClass]['active']) && empty($item[$modelClass]['active'])) {
			$actions[] = $this->Html->link('<i class="icon-ok"></i> '.__('Accepter'), array('action' => 'save_field', $item[$modelClass]['id'], 'active', 1, 'admin' => true), array('class' => 'btn btn-success btn-mini', 'escape' => false));	
		}
		
		$actions[] = $this->Html->link('<i class="icon-edit"></i> '.__('Modifier'), array('action' => 'edit', $item[$modelClass]['id'], 'admin' => true), array('class' => 'btn btn-mini', 'escape' => false));
